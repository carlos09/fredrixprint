jQuery(window).scroll(function(e){ 
  if(jQuery('#wpsl-stores ul li').length){	
  jQueryel = jQuery('.sticky'); 
  if (jQuery(this).scrollTop() > 350 && jQueryel.css('position') != 'fixed'){ 
    jQuery('.sticky').css({'position': 'fixed', 'top': '50px', 'width' : jQuery('#wpsl-wrap').width() / 2, 'margin-left' : jQuery('#wpsl-wrap').width() / 2+'px'}); 
  }
  if (jQuery(this).scrollTop() < 350 && jQueryel.css('position') == 'fixed')
  {
    jQuery('.sticky').css({'position': 'relative', 'top': '0px', 'width' : '50%', 'margin-left' : '0'}); 
  } 
 }
});
...
$( "#wpsl-search-btn" ).on( "click", function() {
	jQuery('#search-message').hide();
	jQuery('#wpsl-gmap').css('opacity','1');
	var keepStartMarker = false;
...
function storeHtml( response, url ) {
	...
	html = "<li data-store-id='" + id + "'><div>" + storeImg + "<h3>" + store + "<span style='float:right;'>"+distance+"</span></h3><span class='wpsl-street'>" + address + "</span>" + address2 + city + " " + state + " " + zip + "<span class='wpsl-country'>" + country  + moreInfo + "</div><a class='wpsl-directions' " + url.target + " href='" + url.src + "'>" + wpslLabels.directions + "</a></li>";
